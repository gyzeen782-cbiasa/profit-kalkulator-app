...
const FilesModule = (() => {
  ...
  let allFiles = [];
  let sortMode = 'terbaru'; // terbaru | terlama | nama
  ...
  function sortedFiles(files) {
    return [...files].sort((a, b) => {
      if (sortMode === 'nama') return a.name.localeCompare(b.name);
  ...
  function renderFiles(filter = '') {
    ...
    const searchVal = filter || document.getElementById('files-search')?.value || '';
    ...
    files.forEach(f => {
      list.appendChild(buildFileCard(f, true));
    });
  }
  ...
  async function renderHome() {
    ...
    document.getElementById('home-total-files').textContent =
      `${allFiles.length} file · ${items.length} barang`;

    // Recent
    const recent = document.getElementById('home-recent');
    const sorted = sortedFiles(allFiles).slice(0, 3);
    recent.innerHTML = '';
    if (sorted.length === 0) {
      recent.innerHTML = '<div style="color:var(--text3);font-size:13px;padding:8px 0;">Belum ada file.</div>';
    } else {
      sorted.forEach(f => recent.appendChild(buildFileCard(f, false)));
    }

    // Preview
    const preview = document.getElementById('home-files-preview');
    preview.innerHTML = '';
    const all = sortedFiles(allFiles).slice(0, 5);
    all.forEach(f => preview.appendChild(buildFileCard(f, false)));
  }
  ...
  function buildFileCard(f, showActions) {
    ...
    // Compute item count + total for this file
    ...
    div.innerHTML = `
      ...
      <div class="file-card-body">
        ...
        <div class="file-card-meta">${itemCount} barang · ${fmtDate(f.updatedAt)}</div>
      </div>
      <div>
  ...
  async function confirmDeleteFile(id, name) {
    App.confirm(`Hapus file "${name}"?`, 'Data barang di dalam file ini juga akan dihapus.', async () => {
